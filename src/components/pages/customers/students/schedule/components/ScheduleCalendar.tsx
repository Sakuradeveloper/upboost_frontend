...
const ScheduleCalendar = ( {events} : Props)  => {
    ...
    const handleAgree = () => {
      ...
      // console.log('Agree clicked with input time:', inputTime);
      try {
          ...
          <DialogContent>
            ...
            {selectedEvent && (
              <Typography variant="body1" color="textSecondary" style={{ marginTop: '20px' }}>
                開始時間: {dayjs(selectedEvent.start).format('YYYY-MM-DD HH:mm')} <br />
                終了時間: {dayjs(selectedEvent.end).format('YYYY-MM-DD HH:mm')}
              </Typography>
            )}
          </DialogContent>
          <DialogActions>
