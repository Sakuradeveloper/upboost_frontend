...
// import { acceptApplication,  fetchApplicationTrial,  fetchNotificationTableAdmin } from '@/store/features/notification';
import { unwrapResult } from '@reduxjs/toolkit';
import { acceptApplicationTrial, fetchApplicationTrial } from '@/store/features/trial';

interface Props {
    search_url?: string;
}

const ApplicationTrialTable = ({ search_url }: Props) => {
    ...
    useEffect(()=>{
        if(user?.id){
            ...
            //     dispatch(fetchNotificationTableAdmin(user?.id))
            dispatch(fetchApplicationTrial(user?.id))
        }
    },[dispatch, user])
    ...
    const handleAccept = async(id:number) => {
        ...
        try {
            ...
            const resultAction = await dispatch(acceptApplicationTrial(id));
...
export default ApplicationTrialTable;
