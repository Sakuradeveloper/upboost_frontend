      ...
      <ChatBody ref={chatBodyRef}>
        {messages?.length > 0 ? (
          messages?.map((message:any, index:number, array:any) => {
            ...
            return <Grid container key={message.id} justifyContent={message.sender === user?.id ? 'flex-end' : 'flex-start'}>
              <Grid item xs={12} sm={8} md={6} display={'flex'} justifyContent={message.sender === user?.id ? 'flex-end' : 'flex-start'} alignItems={'center'}>
                ...
                <MessageBubble isOutgoing={message.sender === user?.id} lastMessage={lastMessage}>
                  <Typography sx={{backgroundColor:`${message.sender === user?.id ? '#cabae4' : '#f7f6fa'}`, px:2, py:1, borderRadius:2, boxShadow: 2 }}>{message.message}</Typography>
